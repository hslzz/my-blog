...
1.3 本站：指由站长负责维护运营的《花生莲子粥の小窝》博客网站（网络域名：blog.hslzz.cn，且该域名所属的主域名、其他子域也适用于本协议）、本站所有的用于提供服务的网络资产、APP、社交平台等，**以下简称“本站”**。包括现有的及未来发布的任何版本（含客户端软件、服务器端软件、源代码、目标代码、资源文件等）；包括相关网络服务器、官方网站、电子媒体、印刷材料等。
...
3.4.6 通过相似读音、字形字等替代，尝试绕过敏感词检测的。
...
6.2 若您的站点违反上述条款的内容，本站不予添加友情链接。若本站友情链接中添加您的站点后，您的站点更新了违反上述申请规定的内容，经核实后站长将撤销您在本站的友情链接且保留自主决定是否通知您的权利。
...
7.2 本站文章均采用国际通用**CC BY-NC-SA 4.0协议**。您可以**以基于CC BY-NC-SA 4.0协议且非商业性质为目的**的转载本站文章或使用进行二次创作，但**必须进行署名**，署名需包含作者（花生莲子粥）及本站相关文章链接。（注：以“基于CC BY-NC-SA 4.0协议转载或二创本站文章意思指：基于本站文章转载或二创后的文章也必须是基于CC BY-NC-SA 4.0协议。）
...
10.6 站长以其能力范围内的最大努力保护您在本站留存的所有信息，但因黑客等站长现有的网络技术无法抗衡的行为导致您的信息泄露，请向作恶方维权，本站不承担责任，请自行做好信息保护。

10.7 因用户误操作导致的损失，本站不承担责任，造成本站损失的，用户需承担所有责任。

10.8 站长有权无理由对任一网络特征实行任何限制或封禁（如人机验证，封禁IP等）。
