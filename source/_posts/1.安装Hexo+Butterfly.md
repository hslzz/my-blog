...
tags:
  ...
  - Butterfly
  - 教程
description: 阅读这篇完整指南，了解如何掌握安装Hexo+Butterfly博客的方法。我们将介绍所有方法，包括：安装node和npm，npm换源，安装Hexo和Butterfly。
---

这篇文章讲解了怎么在Ubuntu和Windows下安装Hexo和Butterfly主题的过程，包括了安装node和npm，npm的换源，最后安装Hexo和Butterfly。
...
我们只需要安装Node，Node依赖于npm，安装Node时它会同时安装好npm
...
```
   ...
   修改策略的命令是 `Set-ExecutionPolicy RemoteSigned`，回车。
...
3. 修改配置文件 __config.yml_，应用Butterfly主题
...
5. 在{blogroot}目录创建_config.butterfly.yml_
   ...
   > 
   >
   > 1. 如果用了 _config.butterfly.yml，那主题目录（不是blogroot目录）的 _config.yml 就没有用了。
